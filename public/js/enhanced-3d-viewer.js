...
class Enhanced3DViewer {
    ...
    async loadHumanModel() {
        // Use Enhanced Mannequin directly for a professional look
        this.createEnhancedMannequin();
    }

    async loadRealisticHumanModel() {
        // Deprecated: Removing robot fallback
        return null;
    }

    createEnhancedMannequin() {
        // Create a professional fashion mannequin
        const humanGroup = new THREE.Group();
        humanGroup.name = 'humanModel';

        // Professional Mannequin Material (Matte White/Grey)
        ...
        // Head (Abstract/Minimalist)
        const headGeometry = new THREE.SphereGeometry(0.24, 64, 64);
        ...
        // Neck
        const neckGeometry = new THREE.CylinderGeometry(0.1, 0.12, 0.25, 32);
        ...
        // Torso (Athletic fit)
        const torsoGeometry = new THREE.CylinderGeometry(0.38, 0.42, 1.1, 32);
        const torso = new THREE.Mesh(torsoGeometry, mannequinMaterial);
        torso.position.y = 0.45;
        torso.castShadow = true;
        torso.receiveShadow = true;
        torso.scale.set(1, 1, 0.75); // Slightly flattened
        humanGroup.add(torso);

        // Shoulders
        const shoulderGeometry = new THREE.SphereGeometry(0.15, 32, 32);
        
        const leftShoulder = new THREE.Mesh(shoulderGeometry, mannequinMaterial);
        leftShoulder.position.set(-0.5, 0.9, 0);
        leftShoulder.castShadow = true;
        humanGroup.add(leftShoulder);

        const rightShoulder = new THREE.Mesh(shoulderGeometry, mannequinMaterial);
        rightShoulder.position.set(0.5, 0.9, 0);
        rightShoulder.castShadow = true;
        humanGroup.add(rightShoulder);

        // Arms (Relaxed pose)
        this.createMannequinArm(humanGroup, -0.6, mannequinMaterial); // Left
        this.createMannequinArm(humanGroup, 0.6, mannequinMaterial);  // Right

        // Hips
        const hipsGeometry = new THREE.CylinderGeometry(0.42, 0.44, 0.25, 32);
        ...
        // Legs
        this.createMannequinLeg(humanGroup, -0.22, mannequinMaterial); // Left
        this.createMannequinLeg(humanGroup, 0.22, mannequinMaterial);  // Right

        // Position the model
        humanGroup.position.y = 0.2; // Lift slightly
        
        this.humanModel = humanGroup;
        this.scene.add(this.humanModel);

        // Initial rotation
        this.humanModel.rotation.y = 0.2;

        console.log('✓ Professional Mannequin loaded');
    }

    createMannequinArm(parentGroup, xOffset, material) {
        const isLeft = xOffset < 0;
        const sign = isLeft ? -1 : 1;

        // Upper arm
        const upperArmGeo = new THREE.CylinderGeometry(0.09, 0.08, 0.5, 16);
        ...
        elbow.position.set(xOffset + (sign * 0.08), 0.35, 0);
        elbow.castShadow = true;
        parentGroup.add(elbow);

        // Lower arm
        const lowerArmGeo = new THREE.CylinderGeometry(0.07, 0.06, 0.45, 16);
        const lowerArm = new THREE.Mesh(lowerArmGeo, material);
        lowerArm.position.set(xOffset + (sign * 0.15), 0.12, 0.05);
        ...
        handGeo.scale(1, 1.5, 0.6);
        const hand = new THREE.Mesh(handGeo, material);
        hand.position.set(xOffset + (sign * 0.22), -0.15, 0.08);
        hand.castShadow = true;
        parentGroup.add(hand);
    }

    createMannequinLeg(parentGroup, xOffset, material) {
        // Upper leg
        const upperLegGeo = new THREE.CylinderGeometry(0.14, 0.12, 0.6, 16);
        ...
        knee.position.set(xOffset, -0.82, 0.02);
        knee.castShadow = true;
        parentGroup.add(knee);

        // Lower leg
        const lowerLegGeo = new THREE.CylinderGeometry(0.11, 0.09, 0.6, 16);
        ...
        foot.position.set(xOffset, -1.48, 0.08);

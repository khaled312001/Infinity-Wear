    ...
    <div class="container">
        ...
        <div class="whatsapp-info">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $whatsappMessage->from_number ?? 'غير محدد' }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">اسم المرسل:</span>
                <span class="info-value">{{ $whatsappMessage->contact_name ?? 'غير محدد' }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">تاريخ الإرسال:</span>
                <span class="info-value">{{ $whatsappMessage->created_at ? $whatsappMessage->created_at->format('Y-m-d H:i') : 'غير محدد' }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">نوع الرسالة:</span>
                <span class="info-value">{{ $whatsappMessage->message_type ?? 'نص' }}</span>
            </div>
        </div>

        <div class="message-content">
            <h3>نص الرسالة</h3>
            <div class="message-text">{{ $whatsappMessage->message ?? 'لا يوجد نص للرسالة' }}</div>
        </div>

        <div style="text-align: center; margin: 30px 0;">
            <a href="{{ url('/admin/whatsapp') }}" class="btn">عرض جميع الرسائل</a>
            <a href="https://wa.me/{{ str_replace(['+', ' ', '-'], '', $whatsappMessage->from_number ?? '') }}" 

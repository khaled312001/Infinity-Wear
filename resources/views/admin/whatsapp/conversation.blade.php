...
<div class="conversation-header p-3 bg-light border-bottom">
    <div class="d-flex align-items-center justify-content-between">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h6 class="mb-0">{{ $contact['name'] ?? 'جهة اتصال غير معروفة' }}</h6>
                <small class="text-muted">{{ $phoneNumber }}</small>
                @if($contact)
                    <span class="badge bg-{{ $contact['type'] === 'importer' ? 'warning' : ($contact['type'] === 'marketing' ? 'info' : ($contact['type'] === 'sales' ? 'success' : 'primary')) }} ms-2">
                        {{ $contact['type'] === 'importer' ? 'مستورد' : ($contact['type'] === 'marketing' ? 'تسويق' : ($contact['type'] === 'sales' ? 'مبيعات' : 'عميل')) }}
                    </span>
                @endif
            </div>
        </div>
        <div class="conversation-actions">
            <button class="btn btn-sm btn-outline-primary" onclick="openWhatsApp('{{ $phoneNumber }}')">
...
<div class="messages-area" style="height: 400px; overflow-y: auto; padding: 1rem;">
    @if($messages->count() > 0)
        @foreach($messages as $message)
            <div class="message-item mb-3 {{ $message->direction === 'outbound' ? 'text-end' : 'text-start' }}">
                <div class="message-bubble d-inline-block p-3 rounded {{ $message->direction === 'outbound' ? 'bg-primary text-white' : 'bg-light' }}" style="max-width: 70%;">
                    ...
                    <div class="message-meta mt-2">
                        <small class="{{ $message->direction === 'outbound' ? 'text-white-50' : 'text-muted' }}">
                            ...
                            @if($message->direction === 'outbound')
                                <i class="fas fa-check{{ $message->status === 'read' ? '-double' : '' }} ms-1"></i>
                            @endif
                        </small>
                    </div>
                </div>
...
<div class="message-input p-3 border-top">
    <form id="sendMessageForm" class="d-flex gap-2">
        <input type="hidden" name="to_number" value="{{ $phoneNumber }}">
        <input type="hidden" name="contact_type" value="{{ $contact['type'] ?? 'external' }}">

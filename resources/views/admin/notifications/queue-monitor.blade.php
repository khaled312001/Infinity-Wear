...
@section('page-subtitle', 'مراقبة حالة نظام الإشعارات والـ Queue')
            ...
            <div class="card stats-card enhanced">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h3 class="stats-number" id="pending-jobs">{{ $stats['pending_jobs'] }}</h3>
            ...
            <div class="card stats-card enhanced">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h3 class="stats-number" id="processed-jobs">{{ $stats['processed_jobs'] }}</h3>
            ...
            <div class="card stats-card enhanced">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h3 class="stats-number" id="failed-jobs">{{ $stats['failed_jobs'] }}</h3>
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h3 class="stats-number" id="worker-status">
                                <span class="badge bg-{{ $stats['worker_status'] == 'running' ? 'success' : 'danger' }}">
                                    {{ $stats['worker_status'] == 'running' ? 'يعمل' : 'متوقف' }}
                                </span>
                            </h3>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <h6>حالة النظام:</h6>
                            <p>
                                <span class="badge bg-{{ $stats['queue_status'] == 'idle' ? 'success' : ($stats['queue_status'] == 'active' ? 'warning' : 'danger') }}">
                                    {{ $stats['queue_status'] == 'idle' ? 'خامل' : ($stats['queue_status'] == 'active' ? 'نشط' : 'مشغول') }}
                                </span>
                            </p>
                        </div>
                        <div class="col-md-6">
...
$(document).ready(function() {
    ...
    $('#retry-failed-jobs').click(function() {
        if (confirm('هل تريد إعادة تشغيل جميع المهام الفاشلة؟')) {
            $.ajax({
                url: '{{ route("admin.notifications.queue-monitor.retry-failed") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
    ...
    $('#flush-failed-jobs').click(function() {
        if (confirm('هل تريد حذف جميع المهام الفاشلة؟ هذا الإجراء لا يمكن التراجع عنه.')) {
            $.ajax({
                url: '{{ route("admin.notifications.queue-monitor.flush-failed") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
    ...
    $('#restart-worker').click(function() {
        if (confirm('هل تريد إعادة تشغيل Queue Worker؟')) {
            $.ajax({
                url: '{{ route("admin.notifications.queue-monitor.restart-worker") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
...
function refreshStats() {
    $.ajax({
        url: '{{ route("admin.notifications.queue-monitor.stats") }}',
        method: 'GET',
        success: function(response) {
            if (response.success) {

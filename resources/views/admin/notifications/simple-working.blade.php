            ...
            <div class="card-body">
                ...
                <div class="notification-tabs mb-4">
                    <ul class="nav nav-pills" id="filter-tabs">
                        <li class="nav-item">
                            <button class="nav-link active" data-filter="all" onclick="switchFilter('all')">
                                <i class="fas fa-list me-2"></i>
                                الكل <span class="badge bg-primary ms-1" id="all-badge">0</span>
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-filter="unread" onclick="switchFilter('unread')">
                                <i class="fas fa-envelope me-2"></i>
                                غير مقروءة <span class="badge bg-danger ms-1" id="unread-badge">0</span>
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-filter="order" onclick="switchFilter('order')">
                                <i class="fas fa-shopping-cart me-2"></i>
                                الطلبات <span class="badge bg-success ms-1" id="orders-badge">0</span>
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-filter="contact" onclick="switchFilter('contact')">
                                <i class="fas fa-envelope me-2"></i>
                                الرسائل <span class="badge bg-info ms-1" id="contacts-badge">0</span>
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-filter="whatsapp" onclick="switchFilter('whatsapp')">
                                <i class="fab fa-whatsapp me-2"></i>
                                الواتساب <span class="badge bg-warning ms-1" id="whatsapp-badge">0</span>
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-filter="importer_order" onclick="switchFilter('importer_order')">
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    showSuccess(data.message || 'تم تحديد جميع الإشعارات كمقروءة');
                    return;
                }
            }
        } catch (apiError) {
...
function updateStats() {
    ...
    // Update badges
    document.getElementById('all-badge').textContent = stats.total || 0;
    ...
    document.getElementById('importer-badge').textContent = stats.importer_orders || 0;
}
...
function updateConnectionStatus(status) {
    ...
    switch(status) {
        ...
        case 'connected':
            ...
            break;
        case 'mock':
            statusElement.classList.add('mock');
...
function debugSystem() {
    console.log('=== Simple Notification System Debug ===');
...
function getMockNotifications() {
    return [
        ...
        },
        {
            ...
            type: 'contact',
            title: 'رسالة جديدة',
            message: 'رسالة تواصل جديدة من سارة أحمد',

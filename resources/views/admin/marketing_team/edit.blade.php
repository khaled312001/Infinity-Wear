...
@section('title', 'تعديل بيانات عضو فريق التسويق')
@section('dashboard-title', 'لوحة الإدارة')
@section('page-title', 'تعديل بيانات عضو فريق التسويق')
@section('page-subtitle', 'تعديل بيانات عضو فريق التسويق: ' . $member->name)
...
@section('content')
    <div class="dashboard-card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.marketing.update', $member->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('admin_id') is-invalid @enderror" 
                                ...
                                @foreach($admins as $admin)
                                    <option value="{{ $admin->id }}" {{ old('admin_id', $member->admin_id) == $admin->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('department') is-invalid @enderror" 
                                   id="department" name="department" value="{{ old('department', $member->department) }}" required>
                            @error('department')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>

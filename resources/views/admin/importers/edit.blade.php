...
@section('title', 'تعديل بيانات المستورد')
@section('dashboard-title', 'لوحة الإدارة')
@section('page-title', 'تعديل بيانات المستورد')
@section('page-subtitle', 'تعديل بيانات المستورد: ' . $importer->name)
    ...
    <div class="dashboard-card">
        <div class="card-header bg-white border-bottom">
            <h5 class="mb-0">
                ...
                تعديل بيانات المستورد
            </h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.importers.update', $importer->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('phone') is-invalid @enderror" 
                                   id="phone" name="phone" value="{{ old('phone', $importer->phone) }}">
                            @error('phone')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('company_name') is-invalid @enderror" 
                                   id="company_name" name="company_name" value="{{ old('company_name', $importer->company_name) }}">
                            @error('company_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3" id="business_type_other_div" style="display: none;">
                            ...
                            <input type="text" class="form-control @error('business_type_other') is-invalid @enderror" 
                                   id="business_type_other" name="business_type_other" value="{{ old('business_type_other', $importer->business_type_other) }}">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" 
                                ...
                                <option value="new" {{ old('status', $importer->status) == 'new' ? 'selected' : '' }}>جديد</option>
                                <option value="contacted" {{ old('status', $importer->status) == 'contacted' ? 'selected' : '' }}>تم التواصل</option>
                                <option value="qualified" {{ old('status', $importer->status) == 'qualified' ? 'selected' : '' }}>مؤهل</option>
                                <option value="proposal" {{ old('status', $importer->status) == 'proposal' ? 'selected' : '' }}>تم تقديم عرض</option>
                                <option value="negotiation" {{ old('status', $importer->status) == 'negotiation' ? 'selected' : '' }}>قيد التفاوض</option>
                                <option value="closed_won" {{ old('status', $importer->status) == 'closed_won' ? 'selected' : '' }}>تم إغلاق الصفقة بنجاح</option>
                                <option value="closed_lost" {{ old('status', $importer->status) == 'closed_lost' ? 'selected' : '' }}>تم إغلاق الصفقة بدون نجاح</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" 
                              id="notes" name="notes" rows="3">{{ old('notes', $importer->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

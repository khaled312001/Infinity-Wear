...
@section('title', 'لوحة المهام - فريق التسويق')
@section('dashboard-title', 'لوحة المهام')
@section('page-title', 'لوحة المهام')
@section('page-subtitle', 'إدارة وتتبع مهام فريق التسويق بنظام Trello')
            ...
            <div class="marketing-kanban-column" data-status="pending">
                <div class="marketing-kanban-header">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <span class="marketing-badge bg-warning">{{ $tasks->where('status', 'pending')->count() }}</span>
                    </div>
                </div>
                <div class="marketing-kanban-body" data-status="pending">
                    @foreach($tasks->where('status', 'pending') as $task)
                        <div class="marketing-task-card" data-task-id="{{ $task->id }}" data-status="pending" draggable="true">
                            <div class="marketing-task-header">
                                <h6 class="marketing-task-title">{{ $task->title }}</h6>
                                <div class="marketing-task-actions">
                                    <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#marketingTaskModal{{ $task->id }}">
                                        <i class="fas fa-ellipsis-v"></i>
                                    </button>
                                </div>
                            </div>
                            <div class="marketing-task-content">
                                <p class="marketing-task-description">{{ Str::limit($task->description, 80) }}</p>
            ...
            <div class="marketing-kanban-column" data-status="in_progress">
                <div class="marketing-kanban-header">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <span class="marketing-badge bg-info">{{ $tasks->where('status', 'in_progress')->count() }}</span>
                    </div>
                </div>
                <div class="marketing-kanban-body" data-status="in_progress">
                    @foreach($tasks->where('status', 'in_progress') as $task)
                        <div class="marketing-task-card" data-task-id="{{ $task->id }}" data-status="in_progress" draggable="true">
                            <div class="marketing-task-header">
                                <h6 class="marketing-task-title">{{ $task->title }}</h6>
                                <div class="marketing-task-actions">
                                    <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#marketingTaskModal{{ $task->id }}">
            ...
            <div class="marketing-kanban-column" data-status="completed">
                <div class="marketing-kanban-header">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <span class="marketing-badge bg-success">{{ $tasks->where('status', 'completed')->count() }}</span>
                    </div>
                </div>
                <div class="marketing-kanban-body" data-status="completed">
                    @foreach($tasks->where('status', 'completed') as $task)
                        <div class="marketing-task-card" data-task-id="{{ $task->id }}" data-status="completed" draggable="true">
                            <div class="marketing-task-header">
                                <h6 class="marketing-task-title">{{ $task->title }}</h6>
                                <div class="marketing-task-actions">
                                    <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#marketingTaskModal{{ $task->id }}">
...
<div class="modal fade" id="marketingTaskModal{{ $task->id }}" tabindex="-1">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                @if($task->status !== 'completed')
                    <form action="{{ route('marketing.tasks.update-status', $task) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PUT')
...
@section('styles')
<link rel="stylesheet" href="{{ asset('css/marketing-kanban.css') }}">
@endsection

@section('scripts')
<script src="{{ asset('js/marketing-kanban.js') }}"></script>
@endsection

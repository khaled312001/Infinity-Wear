...
@section('title', 'طرق الدفع - لوحة تحكم المستورد')
@section('dashboard-title', 'لوحة المستورد')
@section('page-title', 'طرق الدفع')
@section('page-subtitle', 'إدارة طرق الدفع المفضلة لديك')
                            ...
                            <div class="border rounded p-3 mb-3">
                                ...
                                <table class="table table-sm table-borderless">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <code>{{ $bankAccount['account_number'] }}</code>
                                            <button class="btn btn-sm btn-outline-secondary ms-2" onclick="copyToClipboard('{{ $bankAccount['account_number'] }}')">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <code>{{ $bankAccount['iban'] }}</code>
                                            <button class="btn btn-sm btn-outline-secondary ms-2" onclick="copyToClipboard('{{ $bankAccount['iban'] }}')">
                                                <i class="fas fa-copy"></i>
                                            </button>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td><code>{{ $bankAccount['swift_code'] }}</code></td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                                    ...
                                    <div class="card-body d-flex flex-column">
                                        <div class="d-flex align-items-center mb-3">
                                            ...
                                            <div>
                                                ...
                                                <span class="badge bg-{{ $method['status'] == 'active' ? 'success' : 'warning' }}">
                                                    {{ $method['status'] == 'active' ? 'متاح' : 'متاح عند الطلب' }}
                                                </span>
                                            </div>
                                        </div>
                                        
                                        <p class="card-text text-muted mb-3">{{ $method['description'] }}</p>
                                        ...
                                        <div class="mt-auto">
                                            <div class="row text-center mb-3">
                                                ...
                                                <div class="col-6">
                                                    ...
                                                    <strong class="{{ $method['fees'] == 'مجاني' ? 'text-success' : 'text-warning' }}">
                                                        {{ $method['fees'] }}
                                                    </strong>
                                                </div>
                                            </div>
                                            ...
                                            <div class="d-grid gap-2">
                                                @if($method['status'] == 'active')
                                                    <button class="btn btn-outline-primary btn-sm" onclick="selectPaymentMethod('{{ $method['id'] }}')">
                                                        <i class="fas fa-check me-1"></i>
                                                        اختيار
                                                    </button>
                                                @else
                                                    <button class="btn btn-outline-secondary btn-sm" onclick="requestPaymentMethod('{{ $method['id'] }}')">
...
<div class="modal fade" id="addPaymentMethodModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('importers.payment-methods.add') }}">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="method_type" class="form-label">نوع طريقة الدفع <span class="text-danger">*</span></label>
                        <select class="form-select @error('method_type') is-invalid @enderror" 
